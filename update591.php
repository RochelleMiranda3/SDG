...
if (isset($_GET['update'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $update_query = "UPDATE `research5.9.1 `
        SET no_implemented_CapDev = '$no_implemented_CapDev', no_beneficiaries = '$no_beneficiaries', title_ppa = '$title_ppa', description_ppa = '$description_ppa', total_cost = '$total_cost', fund_source = '$fund_source'
        WHERE id = $id";
    ...
    $sql = "SELECT * FROM `research5.9.1 ` WHERE ID = '$id'";
    $select = mysqli_query($conn, $sql);

    if (mysqli_num_rows($select) > 0) {
        while ($row = mysqli_fetch_assoc($select)) {
            $no_implemented_CapDev = $row['no_implemented_CapDev'];
            $no_beneficiaries = $row['no_beneficiaries'];
...
?>
<div class=" w-100 h-auto" id="research591 " style="display: block;">
                        <h2 class="text-center pb-4">Capacity Development Sessions</h2>
                        <form action="" method="post">
                            <div class="input-group mb-3">
                                <span class="input-group-text"><i class=""></i></span>
                                <input type="text" class="form-control" placeholder="Total number of students starting a degree" name="no_implemented_CapDev" value="<?php echo "$no_implemented_CapDev"?>">
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text"><i class=""></i></span>
                                <input type="text" class="form-control" placeholder="Total number of first-generation students starting a degree" name="no_beneficiaries" value="<?php echo "$no_beneficiaries"?>">
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text"><i class=""></i></span>
